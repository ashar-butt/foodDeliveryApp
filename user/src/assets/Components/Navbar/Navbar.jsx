...
const Navbar = () => {
  ...
  const refreshUserData = async () => {
    try {
      ...
      if (storedUser && storedUser !== "undefined" && storedUser !== "null") {
        ...
        const res = await fetch(`http://localhost:5001/users/profile/${userData._id}`);
        if (res.ok) {
  ...
  const navItems = [
    { path: '/restaurants', icon: 'bi-building', label: 'Restaurants' },
    { path: '/offers', icon: 'bi-percent', label: 'Offers' },
    { path: '/cart', icon: 'bi-cart', label: 'Cart', badge: cartCount },
    { path: '/recent-orders', icon: 'bi-list-ul', label: 'Orders' },
    { path: '/my-reviews', icon: 'bi-star-fill', label: 'Reviews' },
    { path: '/help-center', icon: 'bi-headset', label: 'Help' },
  ...
  return (
    <nav className={`navbar navbar-expand-lg navbar-light sticky-top transition-all duration-300 ${
      ...
      <div className="container">
        <Link className="navbar-brand fw-bold text-white d-flex align-items-center" to="/restaurants" style={{ 

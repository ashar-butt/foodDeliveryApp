  ...
  useEffect(() => {
    const fetchOrderCount = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5001/orders/user/${user._id}`);
        if (res.ok) {
            ...
            <div className="col-md-8">
              ...
              <p className="mb-3" style={{ 
                ...
                <span style={{ animation: 'number-glow 2s ease-in-out infinite alternate' }}>
                  {orderCount}
                </span> / {TARGET_ORDERS} orders this month • 
                <span style={{ animation: 'number-glow 2s ease-in-out infinite alternate 0.5s' }}>
                  {deliveredCount}
                </span> delivered
              </p>
            ...
            <div className="col-md-4 text-center">
              <div className="position-relative d-inline-block">
                ...
                <div style={{ 
                  ...
                  animation: orderCount >= 10 ? 'trophy-bounce 2s ease-in-out infinite' : 
                            orderCount >= 8 ? 'star-twinkle 2s ease-in-out infinite' : 
                            orderCount >= 5 ? 'thumbs-wiggle 2s ease-in-out infinite' : 
                            'rocket-launch 2s ease-in-out infinite'
                }}>
                  {orderCount >= 10 ? <i className="bi bi-trophy"></i> : 
                   orderCount >= 8 ? <i className="bi bi-star-fill"></i> : 
                   orderCount >= 5 ? <i className="bi bi-hand-thumbs-up-fill"></i> : 
              ...
              <p className="mb-0 mt-2 fw-semibold" style={{ 
                ...
                {orderCount >= 10 ? 'Outstanding Performance!' : 
                 orderCount >= 8 ? 'Excellent!' : 
                 orderCount >= 5 ? 'Great Job!' : 
                 'Keep Going!'}
              </p>
            </div>
          </div>

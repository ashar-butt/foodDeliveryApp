...
const ResetPassword = () => {
  ...
  const sendResetOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://localhost:5001/auth/send-otp', {
        method: 'POST',
            ...
            <div className="card-body p-4" style={{ background: 'linear-gradient(145deg, #ffffff, #f8f9fa)' }}>
              {step === 1 ? (
                <form onSubmit={sendResetOtp}>
                  ...
                  <button 
                    type="submit" 
                    className={`cool-btn cool-btn-success cool-btn-lg w-100 ${loading ? 'cool-btn-loading' : ''}`}
                    disabled={loading}
                  >
                    {loading ? <><i className="bi bi-arrow-clockwise me-2"></i>Sending...</> : <><i className="bi bi-envelope me-2"></i>Send OTP</>}
                  </button>
                ...
                <form onSubmit={resetPassword}>
                  ...
                  <button 
                    ...
                    disabled={loading}
                  >
                    {loading ? <><i className="bi bi-arrow-clockwise me-2"></i>Resetting...</> : <><i className="bi bi-check-circle me-2"></i>Reset Password</>}
                  </button>
